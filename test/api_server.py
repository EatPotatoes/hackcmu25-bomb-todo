...
def init_database():
    ...
    if bomb_client:
        ...
        listener_thread = threading.Thread(
            target=listen_for_new_bombs, 
            args=(bombs_collection, users_collection), 
            daemon=True
        )
        listener_thread.start()
        print("Database initialized and bomb listener started")
        return True
    return False
...
def add_task():
    ...
    # Get current bomb or create one
    bomb = bombs_collection.find_one(
        {"user_id": user_id, "exploding": False},
        sort=[("start_time", -1)]
    )
    ...
    if bomb:
        # Add wire to existing bomb
        new_wire = make_wire(task_title, timer_set=300)  # 5 minute timer
        bombs_collection.update_one(
            {"_id": bomb['_id']},
            {"$push": {"wires": new_wire}}
        )
    else:
        # Create new bomb with this task
